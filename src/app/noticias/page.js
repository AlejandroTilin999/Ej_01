        ...
        <Container>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
            {sortedNoticias.map((nota, i) => (
              <a
                key={i}
                href={`/noticias/${nota.slug || i}`}
                className="block group no-underline text-inherit"
              >
                <div
                  className="relative rounded-xl overflow-hidden transition-all duration-300 h-[250px] 
      pointer-events-auto sm:pointer-events-none group hover:scale-[1.01] active:scale-[1.01]"
                >
                  <img
                    src={nota.imagen}
                    alt={nota.titulo}
